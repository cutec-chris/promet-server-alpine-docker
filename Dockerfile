FROM alpine 
RUN apk add --no-cache bash wget tar gcc xz postgres-client mysql-client sqlite && \
	 wget http://downloads.free-erp.de/promet-erp-services_7.0.436_amd64-gtk2.deb --level=12278
...
RUN ar p promet-erp-services_7.0.436_amd64-gtk2.deb data.tar.xz | tar -xJ && \
	 rm promet-erp-services_7.0.436_amd64-gtk2.deb && \
